...
def _find_assignment(s: requests.Session, base: str, course: str,
                     assignment: str) -> dict:
    if assignment.isdigit():
        return _json(s.get(
            f"{base}/api/v1/courses/{course}/assignments/{assignment}"))
    matches = _json(s.get(f"{base}/api/v1/courses/{course}/assignments",
                          params={"search_term": assignment,
                                  "per_page": 100}))
    if len(matches) != 1:
        names = ", ".join(f"{a['id']} {a['name']!r}" for a in matches)
        raise SystemExit(f"Expected exactly one assignment matching "
...
def submit(
    ...
    *,
    ...
    if "online_upload" not in a["submission_types"]:
        raise SystemExit(f"{a['name']!r} does not accept uploads "
                         f"(types: {a['submission_types']})")
    ...
    slot = _json(s.post(
        ...
        data={"name": upload_name, "size": file.stat().st_size,
              "content_type": mimetypes.guess_type(upload_name)[0]
              or "application/octet-stream"}))
    up = requests.post(
        slot["upload_url"], data=slot["upload_params"],
    ...
    print(f"submitted attempt {sub['attempt']} at {sub['submitted_at']}"
          f"{late}: "
          + ", ".join(x["display_name"] for x in sub.get("attachments", [])))
